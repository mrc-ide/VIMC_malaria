...
# run postprocessing by parameter draw to see if this is still faster than alternative
# otherwise the dataframes are too large to manage on memory
...
for (draw in c(1:max_draw)){
  ...
  saveRDS(final_output, paste0())
  message(paste0('done with draw ', draw))
}


saveRDS(final_output, 'final_output.rds')
