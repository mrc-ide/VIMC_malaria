...
output_diagnostic<- function(dt, pg, variable, intro_yr, cols= 5, rows= 5, summary = F){
  ...
  if(summary== F){
    dt<- dt |>
      select(country, scenario, age, parameter_draw, year, all_of(variable))

  }
  if (summary == T){
  if(variable == 'cases'){
    dt<- dt |>
      select(iso3c, scenario, parameter_draw, year, all_of(variable))

    median_output<- dt |>
      filter(parameter_draw == 0)

    stochastic_output <- dt |>
      filter(parameter_draw != 0)

    p<-   ggplot()+
      geom_line(data = median_output, mapping = aes(x= year, y= get(variable), color = scenario))  +
      geom_line(data = stochastic_output, mapping = aes(x= year, y= get(variable), color = scenario, group = parameter_draw), alpha = 0.3)  +
      #facet_wrap(~scenario) +
      geom_vline(xintercept= intro_yr, linetype= "dotted", alpha= 2) +
      geom_line(data= site_data$cases_deaths, mapping = aes(x= year, y= wmr_cases), color= 'green')+
      labs(x= 'Time (in years)', y= variable,
           title= paste0(variable, ' over time: ', unique(dt$iso3c), ', ', description),
    ...
    dt<- dt |>
      select(iso3c, scenario, year, all_of(variable), parameter_draw)
    median_output<- dt |>
      filter(parameter_draw == 0)
    ...
    dt<- dt |>
      select(country, scenario, year, all_of(variable), parameter_draw, age)
    median_output<- dt |>
      filter(parameter_draw == 0)
  ...
  p<-   ggplot()+
    geom_line(data = median_output, mapping = aes(x= year, y= get(variable), color = scenario))  +
    geom_line(data = stochastic_output, mapping = aes(x= year, y= get(variable), color = scenario, group = parameter_draw), alpha = 0.3)  +
    facet_wrap_paginate(~age + scenario, scales= 'free',
                        ncol= cols, nrow= rows,
                        page = pg) +
    geom_vline(xintercept= intro_yr, linetype= "dotted") +
    labs(x= 'Time (in years)', y= variable,
...
scaling_diagnostic<- function(dt){
  ...
  stochastic_output <- dt |>
    ...
    p<-   ggplot()+
      geom_line(data = median_output, mapping = aes(x= year, y= pre_scaled_cases, color = scenario))  +
      geom_line(data = stochastic_output, mapping = aes(x= year, y= pre_scaled_cases, color = scenario, group = parameter_draw), alpha = 0.3)  +
      geom_vline(xintercept= intro_yr, linetype= "dotted") +
