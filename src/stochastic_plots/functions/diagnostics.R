...
pull_doses_output <- function(raw_output, processed_output) {
  ...
  if(grepl("rts4", scenario) | grepl("r4", scenario)) {  ## for the booster scenarios
    ...
    doses_per_year <- left_join(doses_per_year, vimc_cohort, by="year") |>
      mutate(doses = .data$rate_dosing * .data$cohort_size)

  } else {   ## for the dose 3 without booster scenarios

    doses_per_year <-raw_output |>
      dplyr::group_by(.data$year) |>
      dplyr::summarise(n_model= mean(.data$n_0_364),    ## average number of people in the eligible age grp (?best way to do this)
                       doses_model= sum(.data$n_pev_epi_dose_3)) |>
...
pull_plotting_data<- function(scenario){
  ...
  test<- raw_output |>
    mutate(year = floor(.data$timestep/365)) |>
    group_by(.data$year) |>
    summarise(n_2_10 = mean(.data$n_730_3649)) |>
    filter(.data$year<=100)
